  ...
  const handleSubmit = async (
    ...
    if (Sid !== null && Spid !== null) {
      if(/[0-9]/.test(Sid) && /[0-9]/.test(Spid))
      {
        ...
        await axios.delete(
          `http://localhost:8080/doctor/${Sid}`,{ data: { patient_id: parseInt(Spid, 10) }},
          customConfig
        );
        props.closeModal();
        alert.success("Doctor Removed Successfully");
        setID("");
        setPID("");
        props.UpdateState();
      }
      else{
        alert.error("Please Enter the correct doctor id or patient id");
      }
    }
    else{
      alert.error("Please enter doctor id or patient id");
    }
  };
